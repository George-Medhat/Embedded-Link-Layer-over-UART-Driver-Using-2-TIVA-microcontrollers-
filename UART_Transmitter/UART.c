...
#define source_MAC_addresses         "00-00-00-00-01"
#define destination_MAC_addresses         "00-00-00-00-02"
...
void encapsulation_sendframe(char data[])
{
    delay();
//Send source MAC addresses
UART5_OutString(source_MAC_addresses);
delay();
delay();
//Send destination MAC addresses
UART5_OutString(destination_MAC_addresses);
delay();

//Send data Length
UART5_FinalOutUHex(strlen(data));
delay();

 //Send data
UART5_OutString(data);
delay();

//Send TX crc
UART_OutString("   Tx_CRC=");
UART_OutUHex(crcSlow(data, strlen(data)));
delay();
UART5_FinalOutUHex(crcSlow(data, strlen(data)));
delay();

}
void UART5_FinalOutUHex(uint32_t x){
